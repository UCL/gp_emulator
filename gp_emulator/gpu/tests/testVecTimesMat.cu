...
void testVecTimesMat(const real *c_vec,const  real *c_matrix, const real *c_res,const int vec_len, const int matrix_nrows, const int matrix_ncols)
{
    ...
    cublasCheckErrors(cublasSetmatrixrix( matrix_nrows, matrix_ncols, sizeof(real), c_matrix_T, matrix_nrows, d_matrix, matrix_nrows));
    
    gpu_vectorTimesmatrixrix( d_matrix, d_vec, d_res, matrix_nrows, matrix_ncols);
